...
public class GamePlayer : Mob
{
    ...
    public int hunger;
    public int hungerRecharge;
    public int hungerRechargeDelay;
    public int hungerDegrade;
    ...
    public override void Eat(int eat)
    {
        if (ImmuneTime > 0)
        {
            return;
        ...
        Level.Add(new TextParticle("" + eat, X, Y, Color.Get(-1, 110, 110, 110)));
    ...
    public override void Update()
    {
        ...
        var hungerInc = Math.Abs(hunger - 10);
        if (Health < MaxHealth && hungerRechargeDelay == 0 && hungerRecharge == 0)
        {
            hungerRechargeDelay = 120 + ((hungerInc) * 10);
        }

        if (hungerRechargeDelay > 0)
        {
            hungerRechargeDelay--;
        }

        if (hungerRechargeDelay == 0 && hunger > 0)
        {
            hungerRecharge++;
            if (IsSwimming())
            {
    ...
    public void GameWon()
    {
        Level.GamePlayer.invulnerableTime = 60 * 5;
        Game.Instance.Won();
    }
}
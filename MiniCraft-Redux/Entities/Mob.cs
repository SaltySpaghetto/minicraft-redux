...
public abstract class Mob : Entity
{
    ...
    public virtual void Eat(int eat)
    {
    }
    ...
    public virtual void DoHurt(int damage, Direction attackDir)
    {
        ...
        {
            int xd = Level.GamePlayer.X - X;
            int yd = Level.GamePlayer.Y - Y;
    ...
    public virtual bool TrySpawn(Level level)
    {
        ...
        if (level.GamePlayer != null)
        {
            int xd = level.GamePlayer.X - xx;
            int yd = level.GamePlayer.Y - yy;
    ...
    public virtual bool TrySpawnAnimal(Level level)
    {
        int x = Random.NextInt(level.Width);
        int y = Random.NextInt(level.Height);
        ...
        if (tile.ID == Tile.Grass.ID || tile.ID == Tile.Flower.ID)
            if (tile.MayPass(level, x, y, this) && !tile.ConnectsToLiquid() && !tile.ConnectsToSand && Game.Instance.state.currentLevel >= 3 )
            {
                X = xx;
                Y = yy;
                return true;

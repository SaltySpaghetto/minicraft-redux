...
public class PersonalCraftingMenu : Menu
{
    ...
    public static IEnumerable<Recipe> GetRecipes(GamePlayer player)
    {
        yield return new FurnitureRecipe<Workbench>(new ResourceItem(Resource.Wood, 20));
        yield return new ResourceRecipeMultiple(Resource.Planks, 4, new ResourceItem(Resource.Wood, 2));
        yield return new ResourceRecipe(Resource.Doors, new ResourceItem(Resource.Wood, 4));
    ...
    public PersonalCraftingMenu(Recipe[] recipes, GamePlayer player)
    {
    ...
    {
        if (input.Menu.Clicked || input.Craft.Clicked)
        {
            game.Menu = null;
        }

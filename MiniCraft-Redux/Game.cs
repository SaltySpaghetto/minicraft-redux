using MiniCraftRedux.Entities;
using MiniCraftRedux.Graphics;
using MiniCraftRedux.Items.Tools;
using MiniCraftRedux.Levels;
...
using Font = MiniCraftRedux.Graphics.Font;
...
{
    public const string Name = "MiniCraft Redux";
    ...
    {
        if (ticks < Time.Morning) ticks = 0; // Error correct
        if (ticks < Time.Day) time = 0; // Morning
        else if (ticks < Time.Evening) time = 1; // Day
        else if (ticks < Time.Night) time = 2; // Evening
        else if (ticks < Time.dayLength) time = 3; // Night
        else
        { // Back to morning
            time = 0;
            ticks = 0;
            Game.Instance.pastDay = true;
        }
        Game.Instance.tickCount = ticks;
    }
    ...
    protected override void Update(float delta)
    {
        ...
        if (state is not null && !state.player.Removed && !state.hasWon)
        {
            ...
            if (Input.Craft.Clicked && menu == null)
            {
                Instance.Menu = new CraftingMenu(PersonalCraftingMenu.GetRecipes(state.player).ToArray(), state.player);
            }
        }
    ...
    public bool LoadGame()
    {
        ...
        {
            state = new();
            state.Deserialize(new Reader(File.OpenRead("worlds/" + worldName + ".dat")));
    ...
    public void SaveGame()
    {
        try
        {
            ...
            using Stream stream = File.Open("worlds/" + worldName + ".dat", FileMode.Create);
    ...
    public void Render()
    {
        if (state is not null)
        {
            ...
            if (state.currentLevel < 3)
            {
                ...
                screen.OverlayOverworld(lightScreen, state.currentLevel, xScroll, yScroll);
        }
    ...
    private void RenderGui()
    {
        if (state is not null)
        {
            for (int y = 0; y < 2; y++)
            {
                for (int x = 0; x < (screen.Width / 8) + 8; x++)
                {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                int hungerInitialX = screen.Width - 80;
                int hungerColor1 = Color.Rgb(102, 34, 0);
                int hungerColor2 = Color.Rgb(240, 185, 70);
                int hungerColor3 = Color.Rgb(195, 150, 95);


                int hungerColor = Color.Get(0, hungerColor1, hungerColor2, hungerColor3);
                if (i < state.player.hunger)
                {
                    screen.Render(hungerInitialX + (i * 8), screen.Height - 16, 2 + 12 * 32, hungerColor, 0);
                }
                else
                {
                    screen.Render(hungerInitialX + (i * 8), screen.Height - 16, 2 + 12 * 32, Color.Get(000, 100, 000, 000), 0);
                }
            ...
            if (showDebug)
            {
                ...
                //Font.Draw($"{state.player.hungerRecharge},{state.player.hungerRechargeDelay}", screen, 0, 16, Color.Get(444));
                Font.Draw($"{Game.time}", screen, 0, 0, Color.Get(444));
            }

...
public abstract class Level : ISerializable, IDeserializable
{
    ...
    public virtual byte GetData(int x, int y, bool getExtra = false)
    {
        if (x < 0 || y < 0 || x >= Width || y >= Height)
        {
            return 0;
        ...
        return Data[x, y].ExtraData;
    }

    public virtual void SetData(int x, int y, byte tileData, byte ?extraData = null)
    {
        if (x < 0 || y < 0 || x >= Width || y >= Height)
        {
            return;
        }
        byte dataExtra = (byte)0;
        if (extraData != null)
            dataExtra = (byte)extraData;
        Data[x, y] = Data[x, y] with { Data = tileData, ExtraData = dataExtra };
    }

    public virtual void Add(Entity entity)
    {
        if (entity is GamePlayer player)
        {
            GamePlayer = player;
        }
    ...
    public virtual void TrySpawnAnimals(int count)
    {
        for (int i = 0; i < count; i++)
        {
            Mob ?mob = null;
